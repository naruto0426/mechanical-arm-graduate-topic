...
ys_bt_new = np.append(ys_bt[tp2],ys_bt[tp2_not]+180)
...
"""
#ys_new1 = -ys_new1
val = np.polyfit(ys_new1,xs_new1,17)  #y=a*x+b
c = np.polyval(val,[-90,0,90])-[555,1198,1840]
#val[-1] = val[-1]-mean(c)
ys_tp = np.arange(min(ys_new1),max(ys_new1),0.1)
#ys_tp = np.arange(-90,180,0.1)
pred = np.polyval(val,ys_tp)
print(np.polyval(val,[-90,0,90,175]))
#plt.scatter(xs,ys,color='blue',s=1)
plt.scatter(xs_new1,ys_new1,color='blue',s=1)
